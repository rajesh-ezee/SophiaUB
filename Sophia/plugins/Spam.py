...
async def spam(_, message):
  global data
  m = message
  if m.command[0] == 'sspam': del data[m.chat.id]
  ...
  if len(message.text.split()) < 2 and hasattr(message, 'reply_to_message'):
...
.dspam - Same as slspam or (slow spam) but it deletes the spam message. used in chatfight rank boosting!
"""