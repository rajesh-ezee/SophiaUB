  ...
  if not await is_admin(chat_id, user_id):
    ...
    try:
      ...
      async for m in Sophia.get_chat_members(chat_id, filter=enums.ChatMembersFilter.BANNED):
        ...
        try:
          await Sophia.unban_chat_member(chat_id, m.user.id)
          unban += 1
        except: pass
...
@Sophia.on_message(filters.command("banall", prefixes=HANDLER) & filters.user(OWN))
  ...
  if not await is_admin(chat_id, user_id):
    ...
    try:
      ...
      for user_id in Members:
        try:
          await Sophia.ban_chat_member(chat_id, user_id)
          await Sophia.unban_chat_member(chat_id, user_id)
          await asyncio.sleep(1.2)
...
@Sophia.on_message(filters.command("kickall", prefixes=HANDLER) & filters.user(OWN))
