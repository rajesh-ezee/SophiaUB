...
async def unban(_, message):
    me = await Sophia.get_chat_member(message.chat.id, "me")
    if message.reply_to_message:
        target = await Sophia.get_chat_member(message.chat.id, message.reply_to_message.from_user.id)
        if message.reply_to_message.from_user.id == me.user.id:
...
@Sophia.on_message(filters.command("kick", prefixes=HANDLER) & filters.user("me"))
async def kick(_, message):
    ...
    if message.reply_to_message:
        ...
        try:
            await Sophia.ban_chat_member(message.chat.id, target.user.id)
            await Sophia.unban_chat_member(message.chat.id, target.user.id)
            await message.reply("✅ Successfully kicked.")
        except Exception as e:
            logging.error(e)
            await message.reply(f"Error: {e}")
