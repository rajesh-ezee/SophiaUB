...
@Sophia.on_message(filters.command(["pin", "unpin"], prefixes=HANDLER) & filters.user(OWN))
async def pin_unpin(_, message):
  if not message.reply_to_message:
    return await message.reply("ℹ️ Please reply to a message to pin/unpin")
  chat = message.chat
  if chat.type != ChatType.PRIVATE:
    member = await Sophia.get_chat_member(chat.id, message.from_user.id)
  ...
  action = Sophia.pin_chat_message if message.command[0] == "pin" else Sophia.unpin_chat_message
  try:
